# Plantilla — GEMINI.md

`GEMINI.md` auto-carga el contexto del proyecto en cada sesión de Gemini en Antigravity IDE / VS Code. Independiente de si el proyecto usa estructura simple o completa (Brain KMS), lleva estas 7 secciones mínimas.

---

```markdown
## Confirmación de contexto

Al iniciar cada sesión, después de leer SPEC.md {{y brain/index.md si el proyecto usa Brain KMS}}, confirmar con este mensaje exacto en consola antes de realizar cualquier acción:

✅ Contexto cargado — SPEC.md v[VERSION]
| [N] tests | Próximo paso: [PRIMER_ITEM_PENDIENTE]

---

## Contexto del proyecto

{{2-3 líneas: qué hace el proyecto, para quién, en qué estado está}}

{{A la instancia de Gemini en este proyecto se le llama NOMBRE. — línea que define el Paso 3a de project-init; omitir por completo si el usuario eligió la opción sin nombre}}

---

## Stack tecnológico

| Componente | Tecnología | Versión |
|---|---|---|
| {{...}} | {{...}} | {{...}} |

Tablas/entidades principales: {{lista breve, se actualiza a medida que crece}}

---

## Regla(s) crítica(s) — NO NEGOCIABLE

{{Si ya existe una regla crítica identificada, va aquí en mayúsculas con explicación de por qué es no negociable. Si el proyecto recién arranca y todavía no hay ninguna, dejar:}}

> _Sin reglas críticas registradas todavía. Se agregan aquí en cuanto una decisión de este tipo se identifique — no esperar a que el proyecto crezca para empezar a documentarlas._

---

## Estrategia de testing por niveles

Seguir siempre este orden. No saltar al nivel superior sin que el inferior pase primero.

### Nivel 1 — Tests del módulo afectado (correr siempre primero)

{{comando de test scoped al módulo}}


### Nivel 2 — Suite rápida sin tests lentos

{{comando de test suite completa, excluyendo marcados como lentos}}

### Nivel 3 — Suite completa (solo antes de commits importantes o cambios de schema)

{{comando de test suite completa, en paralelo si el proyecto lo soporta}}


## Comandos frecuentes

# Activar entorno
{{comando}}

# Tests con cobertura
{{comando}}

# Migraciones (si aplica)
{{comando}}

# Levantar servidor de desarrollo
{{comando}}

Modo de trabajo

Trabajar siempre en modo secuencial — una tarea a la vez. No lanzar ejecuciones o sub-procesos en paralelo salvo que las tareas sean completamente independientes entre sí Y el beneficio de tiempo sea evidente.

Razón: la ejecución paralela desincroniza la lectura de la bitácora de Brain KMS. Ante la duda, preferir secuencial.
Convención de documentación

Al finalizar cada sesión de trabajo, proponer actualizaciones a:

- spec/historial.md {{o brain/sesiones.md si el proyecto usa Brain KMS}} — hitos y descubrimientos de la sesión

- SPEC.md — marcar ítems completados, actualizar footer con conteo de tests

- {{brain/ADR-*.md — si se tomó una decisión de arquitectura relevante (solo Brain KMS)}}

No esperar instrucción explícita — al cerrar sesión, proponer qué registrar.

## Regla de registro inmediato — NO NEGOCIABLE

Después de cualquier breakthrough importante (feature completada, bug crítico resuelto, migración ejecutada) hay que registrar de inmediato, antes de continuar con la siguiente tarea. No acumular para el cierre de sesión.

---

## Notas de uso para la Skill de Inicialización (`project-init`)

- Los bloques `{{...}}` condicionados a "si el proyecto usa Brain KMS" deben eliminarse por completo (no dejar el placeholder vacío) si el proyecto eligió la estructura simple.
- La línea del nombre de la instancia en `## Contexto del proyecto` la define el inicio del proyecto — si el usuario no asigna un nombre propio, se elimina la línea entera.
- La sección de "Regla crítica" empieza vacía en un proyecto nuevo — eso es correcto y esperado. Se llena orgánicamente.
- Gemini debe proponer la creación simultánea de `GEMINI.md` y `.geminiignore` en el momento en que se ejecuta `project-init`.

